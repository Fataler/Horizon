label day_4:
    ...
    R_t ear neutral "В помещении было темно и прохладно. Запах лекарств ударял в нос."
    ...
    cutscene "После \"бури\" мы выползали из своих комнат и прибирали беспорядок."
    $ hide_scene_cosmos()
    # 9 конец
    # 13 начало
    ...
    R_t "Я обратил внимание на приборы. Координаты… О них я тоже забыл напрочь."
    ...
    V "Он и раньше был себе на уме, но это — что-то новенькое."
    ...
    R not_sure "Как будто и вчера, и позавчера… Как я этого не замечал?"
    ...
    R_t serious think "Дольше всего он возился у отсека с баллонами: было слышно грохот и скрежет раскручиваемых вентилей."
    V tricky "Не выпил таблетки, да?"
    R_t angry "Мне было не до шуток: у капитана явно было не всё в порядке."
    R_t think "Я уже успокоился, решив, что сны — просто сны. Теперь понимаю, что нет."
    ...
    R_t very_angry "Мы рванули на помощь, но он толкнул Ирис к стене и побежал к выходу."
...
# Шкаф
# Кажется, здесь только лекарства. 
# Ничего, что нам поможет.

# Балон
# Просто баллоны с сжатым воздухом, ничего интересного.

# Экран для отслеживания состояния пациента. 
...
# I Нет, я не пользуюсь этим сейфом…
# R Что ж…
# Пароль 1734.
...
# R_t Не важно. Времени перебирать нет. Поищем подсказку.
...
# R Надеюсь, это не пригодится.
    ...
    V "Приложи к носу! Я прихватил салфеток в лазарете."
    ...
    R_t ear dissatisfied "Пытаясь дышать через салфетки, мы шли по коридору. Я вдыхал через раз, но лёгкие всё равно наполнялись газом."
